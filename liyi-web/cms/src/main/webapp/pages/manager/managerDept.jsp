...
<div>
  ...
  <table id="tb" border="1" align="center" width="80%">
    <caption style="font-size: 30px;">部门列表展示</caption>
    <tr>
      <td colspan="9">
        <form method="post" action="${path}/manager/deptServlet">
          ...
          部门名称:<input name="name" value="${name}">
          部门编码:<input name="code" value="${code}">
          父部门编码:<input name="parentCode" value="${parentCode}">
          <input type="submit" value="查询" id="mysubmit">
        </form>

        <a href="${path}/manager/deptServlet?action=getDept&curPage=${pageRes.curPage}&pageSize=${pageRes.pageSize}">新增部门</a>
      </td>
    </tr>
    <tr>
      <td>序号</td>
...
<script>
  ...
  $(function (){
    ...
    $("a.deleteClass").click(function () {
      ...
      var url = path+'/manager/deptServlet';
