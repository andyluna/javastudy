...
<div>
  ...
  <table id="tb" border="1" align="center" width="80%">
    <caption style="font-size: 30px;">菜单列表展示</caption>
    <tr>
      <td colspan="9">
        <form method="post" action="${path}/manager/menuServlet">
          ...
          菜单名:<input name="name" value="${name}">
          菜单打开类型:<input name="openType" value="${openType}">
          父菜单ID:<input name="parentId" value="${parentId}">
          <input type="submit" value="查询" id="mysubmit">
        </form>

        <a href="${path}/manager/menuServlet?action=getMenu&curPage=${pageRes.curPage}&pageSize=${pageRes.pageSize}">新增菜单</a>
      </td>
    </tr>
    <tr>
      <td>序号</td>
  ...
  <c:forEach items="${requestScope.pageRes.data}" var="data" varStatus="mystatus">
    <tr>
      ...
      <td><a href="${path}/manager/menuServlet?action=getMenu&id=${data.id}&curPage=${pageRes.curPage}&pageSize=${pageRes.pageSize}">修改</a></td>
...
<script>
  ...
  $(function (){
    ...
    $("a.deleteClass").click(function () {
      ...
      var url = path+'/manager/menuServlet';
